...
/**
 * Critical Path E2E — OD V2 + Unlabeled Folder + Manual Auto-Label
 *
 * Full pipeline (2-phase training):
 *   Sign in → Create OD V2 project → Upload unlabeled folder →
 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: OD V2 + unlabeled manual → train → deploy";

test.describe("Critical Path — OD V2 Unlabeled Manual @critical-path @od", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-OD-V2-UL-Manual-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      version: "v2",
      namePrefix: "E2E-OD-V2-UL-Manual",
    }, consoleErrors);
    ...
    await createLabelingTask(page, labelingTaskCreationPage, "cp-od-v2-ul", consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-OD-V2-UL-Manual",
    });
