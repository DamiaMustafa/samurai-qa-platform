 ...
 *   Sign in → Create OD V1 project → Upload COCO labeled ZIP →
 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: OD V1 + COCO labeled → train → deploy";
...
test.describe("Critical Path — OD V1 Labeled COCO @critical-path @od", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-OD-V1-COCO-Labeled-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      version: "v1",
      namePrefix: "E2E-OD-V1-COCO-Labeled",
    }, consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-OD-V1-COCO",
    });
