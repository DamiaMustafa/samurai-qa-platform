 ...
 *   Sign in → Create OD V1 project → Upload video → Add class names →
 ...
 */
const TEST_TITLE = "full pipeline: OD V1 + video → train → deploy";
...
test.describe("Critical Path — OD V1 Video @critical-path @od @video", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-OD-V1-Video-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-OD-V1-Video",
    });
