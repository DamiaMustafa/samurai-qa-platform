...
/**
 * Critical Path E2E — SEG V2 + Unlabeled Folder + Grounding DINO
 *
 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: SEG V2 + unlabeled grounding dino → train → deploy";

test.describe("Critical Path — SEG V2 Unlabeled Grounding DINO @critical-path @seg", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-SEG-V2-UL-GroundingDino-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      version: "v2",
      namePrefix: "E2E-SEG-V2-UL-GroundingDino",
    }, consoleErrors);
    ...
    await createLabelingTask(page, labelingTaskCreationPage, "cp-seg-v2-gd", consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-SEG-V2-UL-GD",
    });
