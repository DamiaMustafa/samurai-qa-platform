...
/**
 * Critical Path E2E — SEG V2 + Video
 *
 * Full pipeline (2-phase training):
 *   Sign in → Create SEG V2 project → Upload video → Add class names →
 ...
 *   Deploy → Cleanup
 *
 * Video classes: potholes
 */
const TEST_TITLE = "full pipeline: SEG V2 + video → train → deploy";

test.describe("Critical Path — SEG V2 Video @critical-path @seg @video", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-SEG-V2-Video-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    await createLabelingTask(page, labelingTaskCreationPage, "cp-seg-v2-vid", consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-SEG-V2-Video",
    });
