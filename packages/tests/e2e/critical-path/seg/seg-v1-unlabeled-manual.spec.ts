 ...
 * Class: road
 */
const TEST_TITLE = "full pipeline: SEG V1 + unlabeled manual → train → deploy";
...
test.describe("Critical Path — SEG V1 Unlabeled Manual @critical-path @seg", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-SEG-V1-UL-Manual-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      version: "v1",
      namePrefix: "E2E-SEG-V1-UL-Manual",
    }, consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-SEG-V1-UL-Manual",
    });
