 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: CLS V2 + COCO multi-label → train → deploy";
...
test.describe("Critical Path — CLS V2 Labeled COCO Multi @critical-path @cls", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-CLS-V2-COCO-Labeled-Multi-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      namePrefix: "E2E-CLS-V2-COCO-Labeled-Multi",
    }, consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-CLS-V2-COCO-Multi",
    });
