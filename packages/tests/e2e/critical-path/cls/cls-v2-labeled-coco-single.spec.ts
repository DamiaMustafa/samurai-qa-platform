...
/**
 * Critical Path E2E — CLS V2 + Labeled COCO Single-Label
 *
 * Full pipeline (2-phase training):
 *   Sign in → Create CLS V2 project → Upload COCO labeled ZIP →
 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: CLS V2 + COCO single-label → train → deploy";

test.describe("Critical Path — CLS V2 Labeled COCO Single @critical-path @cls", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-CLS-V2-COCO-Labeled-Single-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      namePrefix: "E2E-CLS-V2-COCO-Labeled-Single",
    }, consoleErrors);

    await uploadLabeledZip(page, uploadDatasetPage, {
      zipPath: fixturePath("cls_labeled_coco_single.zip"),
      format: "coco",
    }, consoleErrors);

    await handleLabelingMode(page, labelingModeSelectionPage, "manual", consoleErrors);
    await createLabelingTask(page, labelingTaskCreationPage, "cp-cls-v2-cs", consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-CLS-V2-COCO-Single",
    });
