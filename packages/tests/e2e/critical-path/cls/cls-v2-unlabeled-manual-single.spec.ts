...
/**
 * Critical Path E2E — CLS V2 + Unlabeled Folder + Manual (Single-Label)
 *
 ...
 *   Deploy → Cleanup
 */
const TEST_TITLE = "full pipeline: CLS V2 + unlabeled manual single-label → train → deploy";

test.describe("Critical Path — CLS V2 Unlabeled Manual Single @critical-path @cls", () => {
  ...
  test.beforeEach(async ({ loginPage, page }) => {
    ...
    projectId = undefined;
    projectName = `E2E-CLS-V2-UL-Manual-Single-${new Date().toISOString().substring(0, 10)}-${String(new Date().getHours()).padStart(2, "0")}-${String(new Date().getMinutes()).padStart(2, "0")}`;
  ...
  test(TEST_TITLE, async ({
    ...
    projectId = await createProject(page, projectCreationPage, {
      ...
      namePrefix: "E2E-CLS-V2-UL-Manual-Single",
    }, consoleErrors);
    ...
    await createLabelingTask(page, labelingTaskCreationPage, "cp-cls-v2-uls", consoleErrors);
    ...
    const trainingState = await startTraining(page, fastTrainingFormPage, projectId, consoleErrors, {
      trainingNamePrefix: "E2E-FastTrain-CLS-V2-UL-Manual-Single",
    });
